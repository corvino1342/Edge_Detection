...
lg_k = log_kernel(ksize=ksize, sigma=1.0)
...
g_k1 = gaussian_kernel(ksize=ksize, sigma=1.0)
g_k2 = gaussian_kernel(ksize=ksize, sigma=2.0)
...
qg_k1 = q_gaussian_kernel(ksize=ksize, sigma=1.0, q=0.02)
qg_k2 = q_gaussian_kernel(ksize=ksize, sigma=2.0, q=0.02)
...
for i in range(len(edge_images)):
    plt.ylabel("Time [s]")
    plt.scatter(edge_titles[i], times[i], alpha=0.5, label=edge_titles[i], s=200)
    plt.grid()
